...
def detect_bingo(phrases: list[str], matched_phrases: set[str]) -> bool:
    ...
    if len(phrases) != GRID_SIZE * GRID_SIZE:
        return False

    grid = [phrases[row * GRID_SIZE : (row + 1) * GRID_SIZE] for row in range(GRID_SIZE)]
    ...
    return any(all(phrase in matched_phrases for phrase in line) for line in lines)
